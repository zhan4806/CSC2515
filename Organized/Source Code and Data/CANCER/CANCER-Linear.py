...
file = open('CANCER_linear_result.txt','w') 
script_dir = os.path.dirname(__file__) #<-- absolute dir the script is in
cancer_path = "Classification Dataset/CERVICAL_CANCER/risk_factors_cervical_cancer.txt"
test_ratio=0.2
...
def main():
    train_data, train_target,test_data,test_target=load_data(cancer_path,32)
    ...
    for t in np.arange(0.025, 0.175,0.025):
        start_time = time.time()
   ...
   # Use Mutual Information to filter features
    for t in np.arange(0.025, 0.1,0.025):
        start_time = time.time()

...
file = open('SPECT_Linear_MI_REDO_result.txt','w') 
script_dir = os.path.dirname(__file__) #<-- absolute dir the script is in
spect_path1 = "Classification Dataset/SPECT/SPECT.train"
spect_path2 = "Classification Dataset/SPECT/SPECT.test"
test_ratio=0.2
...
def load_data(rel_path, feature_number):    
    ...
    for path in rel_path:
        ...
        for line in lines:
            elements=line.split(',')
            if(elements[feature_number]!='?'):
                for i in range(0,len(elements)):
                        elements[i]=float(elements[i])
                y.append(elements[0])
                X.append(elements[1:feature_number+1])
...
def main():
    train_data, train_target,test_data,test_target=load_data([spect_path1,spect_path2],22)
...
##        file.write("Execution time is: %s seconds." % (time.time() - start_time)+ "\n"+"\n")
   ...
   # Use Mutual Information to filter features
    for t in np.arange(0.025, 0.125,0.025):
        start_time = time.time()
...
##    # Use Sequential Backward Elimination to find optimal subset
##    start_time = time.time()
##    sbs(train_data, train_target,test_data,test_target)
##    file.write("Execution time is: %s seconds." % (time.time() - start_time)+ "\n"+"\n")
##    
##    # Use Sequential Backward Floating Elimination to find optimal subset
##    start_time = time.time()
##    sbfs(train_data, train_target,test_data,test_target)
##    file.write("Execution time is: %s seconds." % (time.time() - start_time)+ "\n"+"\n")
##
##    # Hybrid: Corr + SBS
##    file.write("Hybrid: Corr + SBS starts:"+"\n")
##    for t in np.arange(0.1, 1.0,0.1):
##        start_time = time.time()
##        index=filter_pcc(train_data,train_target,t)
##        file.write("Corr-Coef threshold is: {}, {} features left.".format(t,len(index))+ "\n")
##        if len(index) != 0:
##            sbs(train_data[:,index], train_target,test_data[:,index],test_target)
##        file.write("Execution time is: %s seconds." % (time.time() - start_time)+ "\n"+"\n")
##        
##    # Hybrid: Corr + SBFS
...
##        file.write("Execution time is: %s seconds." % (time.time() - start_time)+ "\n"+"\n")
    ...
    # Hybrid: MI + SBS
    file.write("Hybrid: MI + SBS starts:"+"\n")
    for t in np.arange(0.025, 0.125,0.025):
        start_time = time.time()
        index=filter_mi(train_data,train_target,t)
        file.write("Mutual Information threshold is: {}, {} features left.".format(t,len(index))+ "\n")

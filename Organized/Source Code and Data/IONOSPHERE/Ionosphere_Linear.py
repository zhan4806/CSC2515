...
file = open('IONOSPHERE_linear_result.txt','w') 
script_dir = os.path.dirname(__file__) #<-- absolute dir the script is in
ionosphere_path = "Classification Dataset/IONOSPHERE/ionosphere.txt"
test_ratio=0.2
...
def load_data(rel_path, feature_number):    
    ...
    for line in lines:
        elements=line.split(',')
        X.append([float(i) for i in elements[0:34]])
        if(elements[feature_number]=='g'):
            y.append(1)
        elif(elements[feature_number]=='b'):
            y.append(-1)
...
def main():
    train_data, train_target,test_data,test_target=load_data(ionosphere_path,34)
